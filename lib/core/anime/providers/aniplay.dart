  ...
  Future<void> getStreams(String episodeId, Function(List<VideoStream> p1, bool p2) update,
    ...
    servers.forEach((it) {
      ...
      resFuture.then((res) {
        ...
        // 1. Coba cari subtitle sesuai preferensi bahasa pengguna
        if (subtitleArr != null && subtitleArr.isNotEmpty) {
          subtitleItem = subtitleArr.where((st) => st['label']?.toString().toLowerCase() == preferredLang).firstOrNull;
        }

        // 2. Jika preferensi Indonesian, coba juga cari dengan label "id" atau "indo"
        if ((subtitleItem == null || subtitleItem.isEmpty) && (preferredLang == "indonesian" || preferredLang == "indonesia")) {
          subtitleItem = subtitleArr?.where((st) {
            final label = st['label']?.toString().toLowerCase() ?? "";
            return label == "id" || label == "indo" || label == "indonesia" || label == "indonesian" || label.contains("indo");
          }).firstOrNull;
        }

        // 3. Fallback ke default
        if (subtitleItem == null || subtitleItem.isEmpty) {
          subtitleItem = subtitleArr?.where((st) => (st['default']) == true).firstOrNull;
        }

        // 4. Fallback ke English
        if (subtitleItem == null || subtitleItem.isEmpty) {
