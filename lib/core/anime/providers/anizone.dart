...
class AniZone extends AnimeProvider {
  ...
  Future<void> getStreams(String episodeId, Function(List<VideoStream> p1, bool p2) update,
    ...
    final bool preferIndonesian = preferredLang == "indonesian" || preferredLang == "indonesia";
    ...
    for (final track in tracks) {
      ...
      // Cek apakah track ini cocok dengan preferensi bahasa
      final bool isPreferred = preferIndonesian
          ? (srclang == "id" || label.contains("indo"))
          : (srclang == "en" || label.contains("english"));

      if (isPreferred && track.attributes.containsKey("src") && track.attributes.containsKey("data-type")) {
        subs.add({'url': track.attributes['src']!, 'type': track.attributes['data-type']!});
      }
    }

    // Fallback ke "en" default jika tidak ada yang cocok
    if (subs.isEmpty) {
      for (final track in tracks) {
