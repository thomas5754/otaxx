...
class _InstagramDownloaderPageState extends State<InstagramDownloaderPage> {
  ...
  Map<String, dynamic>? _mediaData; // DIUBAH: Map bukan List
  ...
  Future<void> _downloadInstagram() async {
    ...
    final apiUrl = Uri.parse(
      "https://api.ryzumi.net/api/downloader/igdl?url=$encodedUrl",
    );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (json['status'] == true && json['data'] != null && (json['data'] as List).isNotEmpty) {
          final firstItem = json['data'][0];

          setState(() {
            _mediaData = { // DIUBAH: Tetap Map, konsisten dengan deklarasi
              'url': firstItem['url'] ?? "",
          ...
          // Kalau media adalah video, inisialisasi player
          if (_mediaData!['type'] == 'video') {
            _initializeVideoPlayer(videoUrl: _mediaData!['url']);
          }
        } else {
          setState(() {
            _errorMessage = "Data Instagram kosong atau tidak valid.";
          });
        }
      } else {
        setState(() {
  ...
  Future<void> _shareVideo() async {
    ...
    try {
      // DIUBAH: Akses sebagai Map, bukan List
      final mediaUrl = _mediaData!['url']; // ❌ Lama: _mediaData![0]['url']
                                      ...
                                      Container(
                                        ...
                                        decoration: BoxDecoration(
                                          ...
                                          image: DecorationImage(
                                            image: NetworkImage(_mediaData!['thumbnail']),
                                            fit: BoxFit.cover,
                                          ),
                                        ),
                                      ),
                              ...
                              _buildGlassCard(
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    Expanded(
                                      child: Text(
                                        "Media berhasil diunduh dari Instagram",
                                        style: TextStyle(
